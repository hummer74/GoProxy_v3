	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "Host ") {
			...
			if current.Name != "" && current.Name != "*" {
				...
				debugLog("PARSER", "Parsed host %s: HostName=%s, User=%s, Port=%s, IdentityFile=%s", current.Name, current.HostName, current.User, current.Port, current.IdentityFile)
			}
			...
			if len(parts) == 2 {
				...
				if current.Params == nil {
					current.Params = make(map[string]string)
				}
				...
				case "proxyjump":
					...
					// No default ignore - all params are stored in Params
				}
			}
		}
	}
