...
//   Section 2 — Hardcoded domain fallback rules
//   Section 3 — GeoIP Russia → DIRECT
//   Section 4 — Private IP safety net → DIRECT
//   Section 5 — Default proxy for everything else
//
// Sections 3 and 4 share a single dnsResolve call (JS var is function-scoped)
...
`)
        ...
        // ── Section 2: Integrated Hardcode Rules (fallback) ──
...
//
// Defence-in-depth: the cleaner is a line-by-line state machine that strips
// any boilerplate duplicating Section 0 (toLowerCase, localhost, dnsResolve,
// function wrappers). Only if-blocks with shExpMatch/dnsDomainIs/return survive.
...
// or are function-level constructs that should never appear in custom rules.
...
// A bare return is one that starts the line (after trim) with "return".
...
// whose body contains no return statement. Handles multi-line blocks
// where "{" may be on a separate line from "if".
func removeEmptyIfBlocks(lines []string) []string {
        ...
        for i < len(lines) {
                ...
                if strings.HasPrefix(trimmed, "if") {
                        // Find the opening brace — it might be on this line or later
                        foundOpen := false
                        j := i
                        for j < len(lines) {
                                ...
                                // Don't search too far — if we hit another if/return, this isn't a block
                                if j > i+50 {
                                        break
                                }
                                j++
                        ...
                        if !foundOpen {
                                // No opening brace found — just a dangling if, keep it
                                out = append(out, lines[i])
                                i++
                                continue
                        }

                        // Find the matching closing brace starting from the line with {
                        ...
                        // Check if the entire block (from if to closing }) contains return
                        fullBlock := ""
                        for x := i; x < k; x++ {
                                fullBlock += lines[x] + "\n"
                        }
                        ...
                        if !strings.Contains(fullBlock, "return") {
                                // Empty if-block — skip entire block
                                i = k
                                continue
                        }
                }
...
// generateGeoIPBlock builds the GeoIP JavaScript block with nested 2nd-octet
// switch for large first-octet groups. Groups with ≥5 entries get a nested
// switch on the second octet; smaller groups stay flat.
// All isInNet calls within a sub-group are sorted by prefix length (larger
// blocks first) so the most common ranges hit fastest.
func generateGeoIPBlock() string {
        ...
        // Groups with ≥5 entries get a nested 2nd-octet switch
        ...
        for _, fk := range firstKeys {
                ...
                if len(group) >= subSwitchThreshold {
                        // ── Large group: nested 2nd-octet switch ──
