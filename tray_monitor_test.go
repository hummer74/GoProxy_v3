...
func TestEstablishConnection_EmptyChain_NoPanic(t *testing.T) {
        defer func() {
                if r := recover(); r != nil {
                        t.Fatalf("expected no panic for empty hosts list, got %v", r)
                }
        }()

        // establishConnection should handle empty hosts gracefully
...
func TestHandleConnectionInteractive_SetTunnelActive(t *testing.T) {
        ...
        if !connState.IsActive() {
                t.Fatal("expected tunnel to be active after handleConnectionInteractive")
        }
        if connState.GetHost() != "testhost" {
                t.Fatalf("expected host to be testhost, got %q", connState.GetHost())
        }
}
...
func TestHandleChainConnectionInteractive_SetTunnelActive(t *testing.T) {
        ...
        if !connState.IsActive() {
                t.Fatal("expected tunnel to be active after handleChainConnectionInteractive")
        }
        if connState.GetHost() != "h1 -> h2" {
                t.Fatalf("expected host to be 'h1 -> h2', got %q", connState.GetHost())
        }
}
